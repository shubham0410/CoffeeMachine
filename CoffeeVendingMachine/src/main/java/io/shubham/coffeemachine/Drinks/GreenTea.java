...
public class GreenTea extends Drink {
    ...
    public GreenTea(Configuration configuration) {
        Map<String, Long> greenTeaIngredients = configuration.getGreenTeaIngredients();
        for (Map.Entry<String, Long> entry : greenTeaIngredients.entrySet()) {
            switch (entry.getKey()) {
                case "hot_water":
